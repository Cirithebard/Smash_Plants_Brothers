...
Scene* selector_scene = nullptr;										//创建所有的场景变量

SceneManager scene_manager;												//实例化场景管理器
...
void flip_atlas(Atlas& src, Atlas dst)
{
	...
	{
		IMAGE img_flpipped;
		flip_image(src.get_img(i), &img_flpipped);
...
void load_game_resources()
{
	...
	loadimage(&img_platform_small, _T("resources/platform_smal1.png"));
	...
	atlas_run_effect.load_from_file(_T("resources/run_effect %d.png"), 4);
	...
	mciSendString(_T("open resources/pea_shoot_1.mp3 alias pea shoot_1"), NULL, 0, NULL); 
	mciSendString(_T("open resources/pea_shoot_2.mp3 alias pea shoot_2"), NULL,0,NULL); 
...
int main()
{
	...
	BeginBatchDraw();													//开始初始化

	menu_scene = new MenuScene();
	game_scene = new GameScene();
	selector_scene = new SelectorScene();
	/*Scene* scene = new MenuScene();	//舍去直接创建scene类型引入菜单场景，而是用scenemanager来处理
